...
func GoogleCallback(mongoClient *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		log.Println("User info response headers:", resp.Header)
		...
		var googleUser struct {
			Sub           string `json:"sub" bson:"sub"` // Fallback to id if sub is missing
			ID            string `json:"id" bson:"id"`   // Map the id field
		...
		// Use id as fallback for sub
		if googleUser.Sub == "" {
			googleUser.Sub = googleUser.ID
		}
		log.Println("Parsed user: sub =", googleUser.Sub, "email =", googleUser.Email, "name =", googleUser.Name, "verified_email =", googleUser.VerifiedEmail)

		if googleUser.Sub == "" || googleUser.Email == "" {
			log.Println("INVALID USER INFO: sub =", googleUser.Sub, "email =", googleUser.Email, "verified_email =", googleUser.VerifiedEmail)
...
func generateStateOauthCookie(c *gin.Context) string {
	...
	c.SetCookie("oauthstate", state, 7200, "/", "localhost", false, false)
	log.Println("Set oauthstate cookie:", state)
	return state
}
